package Testing;

import Logica.*;


import static org.junit.Assert.assertThrows;
import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertTrue;
import static org.junit.Assert.fail;

import java.time.LocalDate;
import java.time.format.DateTimeFormatter;

import org.junit.jupiter.api.MethodOrderer;
import org.junit.jupiter.api.Order;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.TestMethodOrder;

import java.util.Set;
import java.util.Vector;
import java.util.HashSet;
import java.util.Map;
import java.util.HashMap;

import excepciones.KeywordRepetida;
import excepciones.OfertaNoExisteException;
import excepciones.OfertaRepetidaException;
import excepciones.TOfertaRepetidaException;

import excepciones.UsuarioRepetidoException;

@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
public class ContOfertaTest{

    private IContOferta ContO = Fabrica.getInstancia().getIContOferta();
    private IContUsuario ContU = Fabrica.getInstancia().getIContUsuario();
    private ManejadorOferta ManO = ManejadorOferta.getInstancia();
    private ManejadorUsuario ManU = ManejadorUsuario.getInstancia();
    private CargaDatos c = new CargaDatos(ContU, ContO);
    
    DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");

    @Order(1)
    @Test
    public  void AltaDeKeyword() throws KeywordRepetida{
    	ContO.AltaDeKeyword("HomeOffice".toLowerCase());
    	ContO.AltaDeKeyword("Jornada Corta".toLowerCase());
    	
    	assertTrue(ContO.listarKeywords().contains("HomeOffice".toLowerCase()));
    	assertTrue(ContO.listarKeywords().contains("Jornada Corta".toLowerCase()));
    	
    	
    	Exception exceptionKeyRepited = assertThrows(KeywordRepetida.class, () -> {
    		ContO.AltaDeKeyword("Jornada Corta".toLowerCase());
    		});
    	assertTrue(exceptionKeyRepited.getMessage().contains("La keyword ya existe"));
    	
    	
    }
    @Order(2)
    @Test
    public  void agregarTOferta() throws TOfertaRepetidaException{
    	ContO.agregarTOferta("tipouno", "Descripcion TOfertauno.", 20, 20, 100, LocalDate.parse("27/08/2023", dateFormatter));
    	ContO.agregarTOferta("tipob", "Descripcion TOferta.", 2, 31, 300, LocalDate.parse("12/08/2023", dateFormatter));
    	assertTrue(ContO.getNombreTOfertas().contains("tipouno"));
    	assertTrue(ContO.getNombreTOfertas().contains("tipob"));
    }
    @Order(3)
    @Test
    public  void altaOferta() throws OfertaRepetidaException, TOfertaRepetidaException, UsuarioRepetidoException {
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	c.cargarKeywords();
    	Set<String>keys = new HashSet<String>();
    	keys.add("HomeOffice".toLowerCase());
    	keys.add("Jornada Corta".toLowerCase());
    	keys.add("Contabilidad".toLowerCase());
    	
    
    	ContO.altaOferta("antel", "premium", "PlatitaGratis".toLowerCase(), "Queres trabajar? No?! Yo tampoco!.", "10:00", "10:35", 99995, "HomeOffice", "Canelones", LocalDate.parse("27/08/2023", dateFormatter), keys, "", "");
    	assertTrue(ContO.listarOfertas().contains("PlatitaGratis".toLowerCase()));
    	
    	Exception exceptionKeysEmpty = assertThrows(OfertaRepetidaException.class, () -> {
    		ContO.altaOferta("sjobs", "tipoc", "PlatitaGratis2".toLowerCase(), "Queres trabajar? No?! Yo tampoco!.", "10:00", 
    				"10:35", 99995, "HomeOffice", "Canelones", LocalDate.parse("28/08/2023", dateFormatter), null,"","");});
    	assertTrue(exceptionKeysEmpty.getMessage().contains("Falta pasar keywords"));
    	
    	Exception exceptionOferta = assertThrows(OfertaRepetidaException.class, () -> {
    		ContO.altaOferta("sjobs", "tipock", "PlatitaGratis".toLowerCase(), "Queres trabajar? No?! Yo tampoco!.", "10:00", 
    				"10:35", 99995, "HomeOffice", "Canelones", LocalDate.parse("28/08/2023", dateFormatter), null,"","");});
    	assertTrue(exceptionOferta.getMessage().contains("La oferta ya existe"));
    	
    	ManU.clear();
    	ManO.clear();
    }
    

    @Order(4)
    @Test
    public  void modificarOferta(){
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	ContO.modificarOferta("Desarrollador Frontend".toLowerCase());
    	ContO.modificarOferta("PlatitaGratis".toLowerCase());
    	
    	ManO.clear();
    }
    @Order(5)
    @Test
    public  void getOferta() throws OfertaNoExisteException {
    	ManO.clear();
    	ManU.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	assertTrue(ManO.getOfertas().contains(ContO.getOferta("Desarrollador Frontend".toLowerCase())));
    	ManO.clear();
    	ManU.clear();
    }
    
    @Test
    public void getOfertaKeyword() {
    	ManO.clear();
    	ManU.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	String aux = "tiempo completo";
    	assertTrue(ContO.getOfertaKeyword(aux).contains("desarrollador frontend"));
    	ManO.clear();
    	ManU.clear();
    }
    
    @Test
    public void getTOferta() {
    	c.cargarTipoPublicacion();
    	ContO.getTOferta("Premium".toLowerCase());
    	ManO.clear();
    }
    
    @Test
    public void getOfertas() {
    	ManO.clear();
    	ManU.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	assertTrue(ContO.getOfertas().equals(ManO.getOfertas()));
    	ManO.clear();
    	ManU.clear();
    }
    
    @Order(6)
    @Test
    public  void getInfoOfertaString() throws OfertaNoExisteException{
    	ManU.clear();
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	c.cargarPostulaciones();
    	
    	Set<String> res = new HashSet<String>();
    	Set<String> aux = new HashSet<String>();
    	res.add("Nombre: "+"técnico/a básico red".toLowerCase()+"\n"+"Descripcion: "+"REGIMEN DE CONTRATO EN FUNCION PUBLICA EN UN TODO DE ACUERDO A LA NORMATIVA VIGENTE (LEY 16.127, DE 7 DE AGOSTO DE 1990, ART. 1°, LITERAL A) Y B) CON LA MODIFICACION INTRODUCIDA POR EL ART. 11 DE LA LEY 17.930 DE 19 DE DICIEMBRE DE 2005)."+"\n"+"Hora Inicio: "+
    			"09:00"+"\n"+"Hora Fin: "+"17:00"+"\n"+"Departamento: "+
    			"Paysandu "+"\n"+"Ciudad: "+"Paysandu "+"\n"+"Remuneracion: "+"40000.0"+"\n"+"Fecha de Alta: "+"29/09/2023"+"\n"+
    			"Estado: " +"Confirmado"+ "\n" + "Fecha de Aprobacion: "+"29/09/2023"+"\n"+"Tipo de oferta: "+"Premium".toLowerCase()+"\n");
    	res.add("Nombre: "+"analista de datos".toLowerCase()+"\n"+"Descripcion: "+"Ayuda a nuestros clientes a tomar decisiones informadas basadas en análisis y visualizaciones de datos."+"\n"+"Hora Inicio: "+
    			"09:00"+"\n"+"Hora Fin: "+"13:00"+"\n"+"Departamento: "+
    			"Maldonado"+"\n"+"Ciudad: "+"Maldonado"+"\n"+"Remuneracion: "+"40000.0"+"\n"+"Fecha de Alta: "+"19/09/2023"+"\n"+
    			"Estado: " +"Ingresado"+ "\n" + "Fecha de Aprobacion: "+"No Aprobado"+"\n"+"Tipo de oferta: "+"premium".toLowerCase()+"\n"); 	    	
    	for (Oferta i : ManO.getOfertas()) {
    		aux.add(ContO.getInfoOfertaString(i.getNombre()));	
    		}
    	assertTrue(aux.containsAll(res));
    	
    	ManO.clear();
    	ManU.clear();
    }
    @Order(7)
    @Test    
    public  void getInfoPostulantesString() throws OfertaNoExisteException{
    	ManU.clear();
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	c.cargarPostulaciones();
    	
    	Map<String, String> res = new HashMap<String, String>();
    	//res.put("estratega de negocios","Postulante: " + "lgarcia"+ "\n" + "Fecha: "+"02/10/2023"+"\n"+"CV: "+"Licenciada en Administración, experiencia en gestión de equipos y proyectos. Conocimientos en Microsoft Office."+"\n"+"Motivación: "+"Estoy emocionada por la oportunidad de formar parte de un equipo dinámico y contribuir con mis habilidades de liderazgo."+"\n");
    	res.put("estratega de negocios","Postulante: " + "matilo"+ "\n" + "Fecha: "+"30/09/2023"+"\n"+"CV: "+"Estudiante de Comunicación, habilidades en redacción y manejo de redes sociales. Experiencia en prácticas en medios locales."+"\n"+"Motivación: "+"Me encantaría formar parte de un equipo que me permita desarrollar mis habilidades en comunicación y marketing."+"\n"+"Postulante: " + "valen25"+ "\n" + "Fecha: "+"30/09/2023"+"\n"+"CV: "+"Músico profesional, experiencia en espectáculos en vivo. Habilidades en canto y guitarra."+"\n"+"Motivación: "+"Me gustaría combinar mi pasión por la música con una oportunidad laboral que me permita seguir creciendo como artista."+"\n"+"Postulante: " + "lgarcia"+ "\n" + "Fecha: "+"02/10/2023"+"\n"+"CV: "+"Licenciada en Administración, experiencia en gestión de equipos y proyectos. Conocimientos en Microsoft Office."+"\n"+"Motivación: "+"Estoy emocionada por la oportunidad de formar parte de un equipo dinámico y contribuir con mis habilidades de liderazgo."+"\n");
    	res.put("desarrollador frontend","Postulante: " + "maro"+ "\n" + "Fecha: "+"02/10/2023"+"\n"+"CV: "+"Ingeniero en Sistemas, experiencia en desarrollo web y aplicaciones móviles. Conocimientos en JavaScript y React."+"\n"+"Motivación: "+"Me entusiasma la posibilidad de trabajar en proyectos desafiantes y seguir creciendo como profesional en el campo de la tecnología."+"\n"+"Postulante: " + "lgarcia"+ "\n" + "Fecha: "+"01/10/2023"+"\n"+"CV: "+"Licenciada en Administración, experiencia en gestión de equipos y proyectos. Conocimientos en Microsoft Office."+"\n"+"Motivación: "+"Estoy emocionada por la oportunidad de formar parte de un equipo dinámico y contribuir con mis habilidades de liderazgo."+"\n");
    	res.put("diseñador ux/ui","Postulante: " + "javierf"+ "\n" + "Fecha: "+"30/09/2023"+"\n"+"CV: "+"Técnico en Electricidad, experiencia en mantenimiento industrial. Conocimientos en lectura de planos eléctricos."+"\n"+"Motivación: "+"Estoy interesado en formar parte de un equipo que me permita aplicar mis habilidades técnicas y contribuir al mantenimiento eficiente."+"\n");
    	//res.put("estratega de negocios","Postulante: " + "valen25"+ "\n" + "Fecha: "+"30/09/2023"+"\n"+"CV: "+"Músico profesional, experiencia en espectáculos en vivo. Habilidades en canto y guitarra."+"\n"+"Motivación: "+"Me gustaría combinar mi pasión por la música con una oportunidad laboral que me permita seguir creciendo como artista."+"\n");
    	//res.put("desarrollador frontend","Postulante: " + "lgarcia"+ "\n" + "Fecha: "+"01/10/2023"+"\n"+"CV: "+"Licenciada en Administración, experiencia en gestión de equipos y proyectos. Conocimientos en Microsoft Office."+"\n"+"Motivación: "+"Estoy emocionada por la oportunidad de formar parte de un equipo dinámico y contribuir con mis habilidades de liderazgo."+"\n");
    	
    	for (Oferta i : ManO.getOfertas()) {
    		String str = "";
    		if (res.get(i.getNombre()) != null)
    			str = str + res.get(i.getNombre());    		
    		assertTrue(str.equals(ContO.getInfoPostulantesString(i.getNombre())));
    	}
    	ManO.clear();
    	ManU.clear();
    }
    @Order(8)
    @Test
    public  void listarEmpresas(){
    	ManU.clear();
    	ManO.clear();
    	c.cargarUsuarios();
    	Set<String> res = new HashSet<String>();
    	res.add("EcoTech".toLowerCase());
    	res.add("FusionTech".toLowerCase());
    	res.add("GlobalHealth".toLowerCase());
    	res.add("ANTEL".toLowerCase());
    	res.add("MIEM".toLowerCase());
    	res.add("TechSolutions".toLowerCase());
    	assertTrue(res.containsAll(ContO.listarEmpresas()));
    	ManO.clear();
    	ManU.clear();
    }
    @Order(9)
    @Test
    public  void listarTOfertas(){
    	ManO.clear();
    	c.cargarTipoPublicacion();
    	Set<String> res = new HashSet<String>();
    	res.add("Premium".toLowerCase());
    	res.add("Destacada".toLowerCase());
    	res.add("Estandar".toLowerCase());
    	res.add("Basica".toLowerCase());
    	assertTrue(res.containsAll(ContO.listarTOfertas()));
    	ManO.clear();
    	
    }

    @Order(10)
    @Test
    public void agregarNuevoPaqueteTest() throws TOfertaRepetidaException {
    	ContO.agregarPaquete("Paquete1", "Descripción", "10", "20", "27/08/2023","");
        assertTrue(ContO.listarPaquetes().contains("Paquete1"));
    }
    @Order(11)
    @Test
    public void agregarPaqueteRepetidoTest() {
        try {
            ContO.agregarPaquete("Paquete10", "Descripción", "10", "20", "27/08/2023","");
        } catch (TOfertaRepetidaException e) {
            fail("No se esperaba la excepción aquí.");
        }
        assertThrows(TOfertaRepetidaException.class, () -> 
        	ContO.agregarPaquete("Paquete10", "Descripción2", "15", "25", "28/08/2023","")
        );
    }
    @Order(12)
    @Test
    public void listarPaquetesDespuesDeAgregarTest() throws TOfertaRepetidaException {
    	ContO.agregarPaquete("Paquete2", "Descripción", "10", "20", "27/08/2023","");
        assertTrue(ContO.listarPaquetes().contains("Paquete2"));
    }
    @Order(13)
    @Test
    public void getInfoPaqueteTest() throws TOfertaRepetidaException {
        ContO.agregarPaquete("Paquete3", "Descripción Paquete 3", "10", "20", "27/08/2020","");
        String info = ContO.getInfoPaquete("Paquete3");
        assertEquals("Nombre: " + "Paquete3" + "\n" + "Descripcion: " + "Descripción Paquete 3" + "\n" + "Validez: " + 10 + "\n" + "Descuento: " + 20.0 + "\n" + "Costo: " + 0.0 + "\n" + "Fecha de Alta: " + "27/08/2020" + "\n" + "Tipos de Ofertas: " + "\n",info);
        
    }
    @Order(14)
    @Test
	public  void getNombresPaquetesDisponibles(){
    	Set<String> res = new HashSet<String>();
    	res.add("Premium".toLowerCase());
    	res.add("Destacado".toLowerCase());
    	res.add("Express".toLowerCase());
    	res.add("Basica".toLowerCase());
    	assertTrue(res.containsAll(ContO.getNombresPaquetesDisponibles()));
    }

    @Order(15)
    @Test
    public void getNombresTOPaquetesTest() throws TOfertaRepetidaException {
    	ContO.agregarTOferta("Tipo1".toLowerCase(), "Tipo1", 1, 1, 1, LocalDate.parse("27/08/2020", dateFormatter));
    	ContO.agregarPTP("Paquete3", "Tipo1".toLowerCase(), 5);
        Vector<String> tipos = (Vector<String>) ContO.getNombresTOPaquetes("Paquete3");
        assertTrue(tipos.contains("Tipo1".toLowerCase()));
    }

    @Order(16)
    @Test
    public void getInfoTipoTest() {
        String info = ContO.getInfoTipo("Tipo1".toLowerCase());
        assertEquals("Nombre: " + "Tipo1".toLowerCase() + "\n" +
                "Descripcion: " + "Tipo1" + "\n" +
                "Exposicion: " + 1 + "\n" +
                "Duracion: " + 1 + "\n" +
                "Costo: " + 1.0 + "\n" +
                "Fecha de Alta: " + "27/08/2020" + "\n", info);
    }
    
    @Test
    public void getInfoOferta() throws OfertaNoExisteException {
    	ManU.clear();
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	c.cargarPostulaciones();
    	ContO.getInfoOfertaBasico("desarrollador frontend");
    	ContO.getInfoPublicar("desarrollador frontend");
    	ContO.getDTPaquete("premium");
    	ContO.getPaquetesComprados("ecotech");
    	ContO.getCompraPaquetesEmpresa("ecotech");
    	ContO.infoOferta();
    	ContO.infoOfertaConfirmada();
    	ContO.infoPaquetes();
    	ContO.infoTOferta();
    	ContO.infoPaquetesNoComprados("ecotech");
    	ManU.clear();
    	ManO.clear();
    }

 @Order(17)
    @Test
    public void DTOferta() throws OfertaNoExisteException  {
    	ManU.clear();
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	c.cargarPostulaciones();
      DTOferta dtoferta = ContO.getDTOferta("desarrollador frontend");
      assertEquals(dtoferta.getNombre(), "desarrollador frontend");
        
    }
    
    @Order(18)
    @Test
    public void DTPaquete() throws OfertaNoExisteException  {
    	ManU.clear();
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	c.cargarPostulaciones();
      DTPaquete dtpaquete = ContO.getDTPaquete("premium");
      assertEquals(dtpaquete.getNombre(), "premium");
        
    }
    
    @Order(19)
    @Test
    public void DTPublicar() throws OfertaNoExisteException  {
    	ManU.clear();
    	ManO.clear();
    	c.cargarUsuarios();
    	c.cargarTipoPublicacion();
    	c.cargarKeywords();
    	c.cargarPaquetes();
    	c.cargarOfertas();
    	c.cargarPostulaciones();
      DTPublicar dtpublicar = ContO.getOferta("desarrollador frontend").getInfoPublicar();
      assertEquals(dtpublicar.getOferta(), "desarrollador frontend");
        
    }

}
